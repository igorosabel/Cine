...
export default class MoviesService {
    ...
    return this.apiService.getMovies(this.currentPage()).pipe(
      tap((result: MoviesResult): void => {
        this.cachedMovies.update((movies: Movie[]): Movie[] => [
          ...movies,
          ...this.cms.getMovies(result.list),
        ]);
      ...
      catchError((error) => {
        ...
        return throwError(() => new Error('Error al cargar películas'));
      })
    );
  }
  ...
  getMovieById(id: number): Observable<Movie> {
    ...
    return this.apiService.getMovie(id).pipe(
      map((result: MovieResult): Movie => this.cms.getMovie(result.movie)),
      catchError((error) => {
        console.error(`Error al obtener la película con ID ${id}:`, error);
        return throwError(() => new Error('Error al cargar película'));
      })
    );
  }
